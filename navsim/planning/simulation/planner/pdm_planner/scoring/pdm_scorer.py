...
from navsim.planning.simulation.planner.pdm_planner.scoring.pdm_comfort_metrics import ego_is_comfortable, ComfortThresholds
...
class PDMScorer:
    ...
    def score_proposals(
        ...
        self._calculate_ttc(style)
        self._calculate_is_comfortable(style)

        return self._aggregate_scores(style)

    def _aggregate_scores(self, style) -> npt.NDArray[np.float64]:
        """
        ...
        if max_raw_progress > self._config.progress_distance_threshold:
            standard = raw_progress[0]
            actual = raw_progress[1]
            ...
            target = standard * tendency_factors[style]
            score1 = np.exp(-((standard - target) ** 2) / (2 * sigma ** 2))
            score2 = np.exp(-((actual - target) ** 2) / (2 * sigma ** 2))
            normalized_progress = np.array([score1, score2])
        else:
    ...
    def _calculate_ttc(self, style):
        """
        Re-implementation of nuPlan's time-to-collision metric.
        """

        def style_aware_ttc_score(ttc_scores: np.ndarray, style: str) -> np.ndarray:
            if style == "A":
                threshold = 0.8
            elif style == "C":
                threshold = 1.0
            ...
            scores = np.minimum(ttc_scores / threshold, 1.0)
            return scores

        ttc_scores = np.ones(self._num_proposals, dtype=np.float64) * (
            self.proposal_sampling.interval_length * self.proposal_sampling.num_poses
        )
        ...
        future_time_idcs = np.arange(0, 15, 3)
        ...
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                ...
                if current_time_idx >= len(self._observation._occupancy_maps):
                    break
                ...
                for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                    ...
                    if is_agent_ahead(ego_rear_axle, track_state) or (
                        ...
                        collision_time = float(current_time_idx) * self.proposal_sampling.interval_length
                        ttc_scores[proposal_idx] = np.minimum(ttc_scores[proposal_idx], collision_time)
        ...
        regulated_ttc_score = style_aware_ttc_score(ttc_scores, style)

        self._weighted_metrics[WeightedMetricIndex.TTC] = regulated_ttc_score

    def _calculate_is_comfortable(self, style) -> None:
        """
        ...
        if style == 'A':
            style_comfort_thresholds = ComfortThresholds(
                max_abs_mag_jerk=9.02,
                max_abs_lat_accel=5.19,
                max_lon_accel=2.71,
                min_lon_accel=-4.02,
                max_abs_yaw_accel=2.28,
                max_abs_lon_jerk=4.54,
                max_abs_yaw_rate=1.01
            )
        elif style == 'C':
            style_comfort_thresholds = ComfortThresholds(
                max_abs_mag_jerk=5.02,
                max_abs_lat_accel=2.8,
                max_lon_accel=1.4,
                min_lon_accel=-1.9,
                max_abs_yaw_accel=1.0,
                max_abs_lon_jerk=2.5,
                max_abs_yaw_rate=0.5
            )
        else:
            style_comfort_thresholds = ComfortThresholds()

        is_comfortable = ego_is_comfortable(self._states, time_point_s, style_comfort_thresholds)
