...
class ComfortThresholds:
    max_abs_mag_jerk: float = 7.37
    max_abs_lat_accel: float = 4.03
    max_lon_accel: float = 2.02
    min_lon_accel: float = -3.45
    max_abs_yaw_accel: float = 1.73
    max_abs_lon_jerk: float = 3.84
    max_abs_yaw_rate: float = 0.80
...
def _compute_lon_acceleration(
    states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64], thresholds: ComfortThresholds
) -> npt.NDArray[np.bool_]:
    """
    ...
    return _within_bound(lon_acceleration, min_bound=thresholds.min_lon_accel, max_bound=thresholds.max_lon_accel)
...
def _compute_lat_acceleration(
    ...
    return _within_bound(lat_acceleration, min_bound=-thresholds.max_abs_lat_accel, max_bound=thresholds.max_abs_lat_accel)
...
def _compute_jerk_metric(
    ...
    return _within_bound(jerk_metric, min_bound=-thresholds.max_abs_mag_jerk, max_bound=thresholds.max_abs_mag_jerk)
...
def _compute_lon_jerk_metric(
    ...
    return _within_bound(lon_jerk_metric, min_bound=-thresholds.max_abs_lon_jerk, max_bound=thresholds.max_abs_lon_jerk)


def _compute_yaw_accel(states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64], thresholds: ComfortThresholds) -> npt.NDArray[np.bool_]:
    """
    ...
    return _within_bound(yaw_accel_metric, min_bound=-thresholds.max_abs_yaw_accel, max_bound=thresholds.max_abs_yaw_accel)


def _compute_yaw_rate(states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64], thresholds: ComfortThresholds) -> npt.NDArray[np.bool_]:
    """
    ...
    return _within_bound(yaw_rate_metric, min_bound=-thresholds.max_abs_yaw_rate, max_bound=thresholds.max_abs_yaw_rate)


def ego_is_comfortable(states: npt.NDArray[np.float64], time_point_s: npt.NDArray[np.float64], thresholds: ComfortThresholds) -> npt.NDArray[np.bool_]:
    """
    ...
    for idx, metric_function in enumerate(comfort_metric_functions):
        results[:, idx] = metric_function(states, time_point_s, thresholds)
