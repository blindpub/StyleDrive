...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        style_path_root: Optional[str] = None,
    ):
        super().__init__()
        ...
        if self._with_style and style_path_root:
            print("You are caching datasets with driving style feature.")
            self._style_path_root = style_path_root

            with open(self._style_path_root, 'r', encoding='utf-8') as f:
                style_dict = json.load(f)
            
            self._style_data = {key: value["ANC_result"] for key, value in style_dict.items()}
    ...
    def _cache_scene_with_token(self, token: str) -> None:
        ...
        if self._with_style:
            style_idx = STYLE_MAP.get(self._style_data[token], 1)  # default "N"
        ...
        for builder in self._feature_builders:
            ...
            if self._with_style:
                data_dict["style_feature"] = style_feature
