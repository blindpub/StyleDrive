...
styletest_json_path = "/data_storage/jingbowen/projects/friday_project/results/test_scene_attribute_merged.json"
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            traj_save_root = os.path.join(cfg.output_dir, "traj_and_metric")
            os.makedirs(traj_save_root, exist_ok=True)
            traj_save_path = os.path.join(traj_save_root, token + '.pkl')
            data_dict = {
            ...
            # traj_save_root = "/data_storage/haoruiyang/projects/mme2e/StyleDrive/DiffusionDrive/exp/eval_ego_mlp_style_agent/2025.05.16.02.05.34/traj_and_metric"
            # traj_save_path = os.path.join(traj_save_root, token + '.pkl')

            # with open(traj_save_path, 'rb') as f:
            #     data_dict = pickle.load(f)

            style = STYLETEST_LABEL[token]
...
def main(cfg: DictConfig) -> None:
    ...
    cfg.output_dir = cfg_copy["output_dir"]
    ...
    # score_rows: List[Dict[str, Any]] = run_pdm_score(data_points)
