...
class V2TransfuserModel(nn.Module):
    ...
    def __init__(self, config: TransfuserConfig):
        ...
        self._with_style = self._config.with_style

        if self._with_style:
            self._style_embedding = nn.Embedding(3, config.tf_d_model)
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]=None) -> Dict[str, torch.Tensor]:
        ...
        if self._with_style:
            style_feature: torch.Tensor = features["style_feature"]
        ...
        if self._with_style:
            style_id = style_feature.argmax(dim=-1)  # [B]
            style_embed = self._style_embedding(style_id).unsqueeze(1)  # [B, 1, D]
            trajectory_query = trajectory_query + style_embed  # broadcasting add
        
        # if self._with_style:
        #     style_feature = style_feature.unsqueeze(1).expand(-1, trajectory_query.shape[1], -1)  # [B, T, 3]

        #     zero_padding = torch.zeros(
        #         style_feature.size(0),  # batch_size
        #         style_feature.size(1),  # seq_len
        #         self._config.tf_num_head-3,
        #         device=style_feature.device
        #     )  # [B, T, 5]

        #     style_padded = torch.cat([style_feature, zero_padding], dim=-1)

        #     trajectory_query = torch.cat([trajectory_query, style_padded], dim=-1)
